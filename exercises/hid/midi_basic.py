...
if __name__ == "__main__":
    ...
    midi_reader = MIDIDeviceReader()
    
    try:
        # List available devices
        devices = midi_reader.list_devices()
        
        if devices:
            # Connect to first device (or change index)
            device_index = 0
            if midi_reader.connect(device_index):
                
                # Define a custom callback (optional)
                def my_callback(midi_data, parsed_message, timestamp):
                    # You can add custom processing here
                    pass
                
                # Start listening (this will block until Ctrl+C)
                midi_reader.start_listening(callback=my_callback)
